...
if [ ! -d "$SSH_DIR" ]; then
    echo "[SSH-authorized-keys.sh] Creando $SSH_DIR"
    mkdir -p "$SSH_DIR"
    echo "[SSH-authorized-keys.sh] Asignando permisos y propietario a $SSH_DIR"
    chown linuxgsm:linuxgsm "$SSH_DIR"
    chmod 700 "$SSH_DIR"
else
    echo "[SSH-authorized-keys.sh] $SSH_DIR ya existe"
fi

# Si SSH_KEY está vacío, eliminar authorized_keys
if [ -z "$SSH_KEY" ]; then
    echo "[SSH-authorized-keys.sh] SSH_KEY vacío. Eliminando $AUTH_KEYS si existe."
    rm -f "$AUTH_KEYS"
    exit 0
fi
...
echo "[SSH-authorized-keys.sh] Escribiendo claves en $AUTH_KEYS:"
> "$AUTH_KEYS"
for key in "${KEYS[@]}"; do
...
chmod 600 "$AUTH_KEYS"
echo "[SSH-authorized-keys.sh] Operación completada."
